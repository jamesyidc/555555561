...
def get_coin_change_history():
    ...
    try:
        ...
        if not date_str:
            ...
            month_str = beijing_time.strftime('%Y%m')
        else:
            ...
            # 提取月份
            month_str = file_date_str[:6]  # 例如: 202602
        
        # 优先尝试读取统一JSONL文件（新格式）
        unified_file = data_dir / f'coin_change_tracker_{month_str}.jsonl'
        
        if unified_file.exists():
            # 读取统一JSONL文件，筛选指定日期的记录
            records = []
                ...
                for line in f:
                    if line.strip():
                        try:
                            record = json.loads(line.strip())
                            # 检查日期是否匹配
                            if record.get('date') == file_date_str:
                                # 保留完整的统一格式，同时添加兼容字段
                                # 保留原始数据结构
                                unified_record = record.copy()
                                
                                # 添加兼容字段（前端可能仍在使用这些字段）
                                if 'summary' in record:
                                    unified_record['cumulative_pct'] = record['summary'].get('cumulative_pct', 0)
                                    unified_record['total_change'] = record['summary'].get('total_change', 0)
                                    unified_record['up_ratio'] = record['summary'].get('up_ratio', 0)
                                    unified_record['up_coins'] = record['summary'].get('up_coins', 0)
                                    unified_record['down_coins'] = record['summary'].get('down_coins', 0)
                                
                                # 添加count字段
                                unified_record['count'] = len(record.get('coins', {}))
                                
                                # 转换coins为changes格式（兼容旧前端）
                                if 'coins' in record:
                                    changes = {}
                                    for symbol, coin_data in record['coins'].items():
                                        changes[symbol] = {
                                            'current_price': coin_data.get('price', 0),
                                            'baseline_price': coin_data.get('baseline', 0),
                                            'change_pct': coin_data.get('change_pct', 0)
                                        }
                                    unified_record['changes'] = changes
                                
                                # 如果有RSI数据，添加total_rsi字段
                                if 'rsi_summary' in record and record['rsi_summary']:
                                    unified_record['total_rsi'] = record['rsi_summary'].get('total_rsi', 0)
